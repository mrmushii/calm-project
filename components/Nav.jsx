  ...
  return (
    ...
    <div className="flex justify-between items-center px-4 md:px-12 py-4">
      ...
      <div className="hidden md:flex gap-6 md:gap-10 items-center">
        ...
        <Link href="/try_for_free">
          <Button className={` ${btnGra} py-6 rounded-4xl text-white`}>
    ...
    {menuOpen && (
      <div className="md:hidden flex flex-col gap-4 px-4 pb-4">
        <Link href="/sleep" onClick={() => setMenuOpen(false)}>Sleep</Link>
        <Link href="/stress_anxiety" onClick={() => setMenuOpen(false)}>Stress & Anxiety</Link>
        <Link href="/mind" onClick={() => setMenuOpen(false)}>Mindfulness</Link>
        <Link href="/login" onClick={() => setMenuOpen(false)}>Log in</Link>
        <Link href="/try_for_free" onClick={() => setMenuOpen(false)}>
          <Button className={`w-full bg-gradient-to-r ${btnGra} py-2 rounded-3xl text-white`}>
            Try Safe Space for Free
          </Button>
        </Link>
      </div>
